...
function BrandsCountPage() {
  ...
  useEffect(() => {
    setLoading(true);
    GET(`/brandsCount?sort=${sortingOption}`)
        .then((result) => {
            if (result) {
                const apidata = result.map((vehicle) => {
                    return {
                        brand_name: vehicle[0],
                        country_name: vehicle[1],
                        num_brands: vehicle[2],
                    };
                });
                setProcData(apidata);
            } else {
                ...
                <TableBody>
                  {currentItems.map((item, index) => (
                    <TableRow key={index}>
                      ...
                      <TableCell>{item.num_brands}</TableCell>
                    </TableRow>
                  ))}
                </TableBody>
              </Table>
...
export default BrandsCountPage;